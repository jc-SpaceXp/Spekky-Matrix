...
void real_fft_to_db_fs(const float* src, float* dst, unsigned int real_fft_size)
{
	...
	// where max_ref == maximum value of mic output
	// e.g. all bits set on SD out (plus one for 0 value too)
	//unsigned int max_ref = 1 << 23; // SD is 24 bits wide
	// FFT is float32, therefore max reference magnitude is 2^31 - 1
	unsigned int fft_max_ref = INT_MAX;
