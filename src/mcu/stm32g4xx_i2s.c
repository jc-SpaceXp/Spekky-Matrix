...
static void i2s_gpio_setup(void)
{
	...
	// Set all to inputs
	I2S_SD_PORT->MODER &= ~eGET_REG(GPIO_MODER_MODE, I2S_WS_PIN);
	I2S_WS_PORT->MODER &= ~eGET_REG(GPIO_MODER_MODE, I2S_WS_PIN);
	I2S_CLK_PORT->MODER &= ~eGET_REG(GPIO_MODER_MODE, I2S_CLK_PIN);
	// Set I2S pins to AF mode (0b10)
	I2S_SD_PORT->MODER |= eGET_REG_BIT1(GPIO_MODER_MODE, I2S_SD_PIN);
	I2S_WS_PORT->MODER |= eGET_REG_BIT1(GPIO_MODER_MODE, I2S_WS_PIN);
	I2S_CLK_PORT->MODER |= eGET_REG_BIT1(GPIO_MODER_MODE, I2S_CLK_PIN);
	// Use SPI alternative function (SPI2 and I2S share same pins/registers)
	I2S_SD_PORT->AFR[1] |= GPIO_AF5_SPI2 << eGET_AFRH_REG(GPIO_AFRH_AFSEL, I2S_SD_PIN);
	I2S_WS_PORT->AFR[0] |= GPIO_AF5_SPI2 << eGET_AFRL_REG(GPIO_AFRL_AFSEL, I2S_WS_PIN);
	I2S_CLK_PORT->AFR[0] |= GPIO_AF5_SPI2 << eGET_AFRL_REG(GPIO_AFRL_AFSEL, I2S_CLK_PIN);
	// All GPIO ports/pins are push-pull by default (no need for OTYPER)
	// High speed pins
	I2S_SD_PORT->OSPEEDR |= eGET_REG_BIT1(GPIO_OSPEEDR_OSPEED, I2S_SD_PIN);
	I2S_WS_PORT->OSPEEDR |= eGET_REG_BIT1(GPIO_OSPEEDR_OSPEED, I2S_WS_PIN);
	I2S_CLK_PORT->OSPEEDR |= eGET_REG_BIT1(GPIO_OSPEEDR_OSPEED, I2S_CLK_PIN);
}
