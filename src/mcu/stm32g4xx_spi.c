...
static void spi_gpio_setup(void)
{
	...
	// SPI setup
	// Set all to inputs
	SPI_CLK_PORT->MODER &= ~eGET_REG(GPIO_MODER_MODE, SPI_CLK_PIN);
	SPI_MISO_PORT->MODER &= ~eGET_REG(GPIO_MODER_MODE, SPI_MISO_PIN);
	SPI_MOSI_PORT->MODER &= ~eGET_REG(GPIO_MODER_MODE, SPI_MOSI_PIN);
	// SPI1 to AF
	SPI_CLK_PORT->MODER |= eGET_REG_BIT1(GPIO_MODER_MODE, SPI_CLK_PIN);
	SPI_MISO_PORT->MODER |= eGET_REG_BIT1(GPIO_MODER_MODE, SPI_MISO_PIN);
	SPI_MOSI_PORT->MODER |= eGET_REG_BIT1(GPIO_MODER_MODE, SPI_MOSI_PIN);
	// Use SPI alternative function
	SPI_CLK_PORT->AFR[0] |= GPIO_AF5_SPI1 << eGET_AFRL_REG(GPIO_AFRL_AFSEL, SPI_CLK_PIN);
	SPI_MISO_PORT->AFR[0] |= GPIO_AF5_SPI1 << eGET_AFRL_REG(GPIO_AFRL_AFSEL, SPI_MISO_PIN);
	SPI_MOSI_PORT->AFR[0] |= GPIO_AF5_SPI1 << eGET_AFRL_REG(GPIO_AFRL_AFSEL, SPI_MOSI_PIN);
	// All GPIO ports/pins are push-pull by default (no need for OTYPER)
	// High speed pins
	SPI_CLK_PORT->OSPEEDR |= eGET_REG_BIT1(GPIO_OSPEEDR_OSPEED, SPI_CLK_PIN);
	SPI_MISO_PORT->OSPEEDR |= eGET_REG_BIT1(GPIO_OSPEEDR_OSPEED, SPI_MISO_PIN);
	SPI_MOSI_PORT->OSPEEDR |= eGET_REG_BIT1(GPIO_OSPEEDR_OSPEED, SPI_MOSI_PIN);
	// Clear reset bit on B4 (MISO, no pull-up or pull-down)
	SPI_MISO_PORT->PUPDR &= ~eGET_REG(GPIO_PUPDR_PUPD, SPI_MISO_PIN);

	// GPIO setup
	// Set all to inputs
	SPI_CS_PORT->MODER &= ~eGET_REG(GPIO_MODER_MODE, SPI_CS_PIN);
	SPI_OE_PORT->MODER &= ~eGET_REG(GPIO_MODER_MODE, SPI_OE_PIN);
	// Set GPIO to outputs
	SPI_CS_PORT->MODER |= eGET_REG_BIT0(GPIO_MODER_MODE, SPI_CS_PIN);
	SPI_OE_PORT->MODER |= eGET_REG_BIT0(GPIO_MODER_MODE, SPI_OE_PIN);
	// All GPIO ports/pins are push-pull by default (no need for OTYPER)
	// High speed pins
