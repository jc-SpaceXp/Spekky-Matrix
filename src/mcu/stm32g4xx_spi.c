...
static void spi_gpio_setup(void)
{
	...
	GPIOA->MODER &= ~ (eGET_REG(GPIO_MODER_MODE, SPI_CS_PIN) | eGET_REG(GPIO_MODER_MODE, SPI_OE_PIN));
	...
	GPIOA->MODER |= eGET_REG_BIT0(GPIO_MODER_MODE, SPI_CS_PIN)
	                ...
	                | eGET_REG_BIT1(GPIO_MODER_MODE, SPI_OE_PIN);
	...
	GPIOA->OSPEEDR |= eGET_REG_BIT1(GPIO_OSPEEDR_OSPEED, SPI_CS_PIN) | eGET_REG_BIT1(GPIO_OSPEEDR_OSPEED, SPI_OE_PIN);
